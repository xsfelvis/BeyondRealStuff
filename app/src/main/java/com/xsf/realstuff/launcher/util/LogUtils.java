package com.xsf.realstuff.launcher.util;
...
public class LogUtils {
     ...
     * 写入日志到文件的线程池
     */
    private static ExecutorService mExecutorServicePool = Executors.newFixedThreadPool(3);
...
//            }
    ...
    private static String[] processContents(int type, String tag, Object... contents) {
        ...
        if (!sTagIsSpace) {
            ...
            // 全局tag为空时，如果传入的tag为空那就显示类名，否则显示tag
    ...
    private synchronized static void print2File(final String tag, final String msg) {
        ...
        mExecutorServicePool.execute(new Runnable() {
            @Override
            public void run() {
